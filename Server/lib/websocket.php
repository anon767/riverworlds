...
class PHPWebSocket {
    ...
    function wsStartServer($host, $port, $callback) {
        ...
        while (isset($this->wsRead[0])) {
            $changed = $this->wsRead;
            $result = socket_select($changed, $write, $except, floor(1 / TICKRATE), ceil(1 / TICKRATE * 1000000));
        ...
        if ($opcode == self::WS_OPCODE_PING) {
            ...
            if (array_key_exists('message', $this->wsOnEvents))
                foreach ($this->wsOnEvents['message'] as $func)
                    $func($clientID, $data, $dataLength, $opcode == self::WS_OPCODE_BINARY, $this);
        }
        else {
            // unknown opcode
            return false;
